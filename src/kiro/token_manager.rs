...
impl TokenManager {
    ...
    pub fn new(config: Config, credentials: KiroCredentials, credentials_path: impl Into<PathBuf>) -> Self {
        Self {
            config,
            credentials,
    ...
    pub async fn ensure_valid_token(&mut self) -> anyhow::Result<String> {
        if is_token_expired(&self.credentials) || is_token_expiring_soon(&self.credentials) {
            ...
            // 刷新成功后持久化到文件
            if let Err(e) = self.credentials.save(&self.credentials_path) {
                tracing::warn!("保存凭证到文件失败: {}，服务将继续运行", e);
            } else {
                tracing::info!("凭证已保存到文件: {:?}", self.credentials_path);
            }
        }
...
mod tests {
    ...
    fn test_token_manager_new() {
        ...
        let tm = TokenManager::new(config, credentials, "credentials.json");
